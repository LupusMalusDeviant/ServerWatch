...
public sealed class AllInOnePseudoModule : IWhiskersModule
{
    ...
    public IReadOnlyList<Type> McpToolTypes { get; } = new[]
    {
        ...
        typeof(SelfStatusTools), typeof(HostLoadTools), typeof(ActionOutcomeTools),
    ...
    public IReadOnlyList<NavItem> NavItems { get; } = new[]
    {
        ...
        new NavItem("notifications", "Nav_Notifications", Icons.Material.Filled.Notifications, "Übersicht",     AppRole.Viewer, 70),
        new NavItem("audit-log",   "Nav_AuditLog",    Icons.Material.Filled.History,      "Übersicht",        AppRole.Viewer, 80),

        // Deployment ("deploy" + "apps" extracted to Modules/Deployment; "compose" stays Core — RoadToSAP Phase 1)
        new NavItem("compose",     "Nav_ComposeEditor",     Icons.Material.Filled.EditNote,     "Deployment",       AppRole.Viewer, 120),

        // Infrastruktur
        new NavItem("servers",     "Nav_Servers",             Icons.Material.Filled.Storage,      "Infrastruktur",    AppRole.Viewer, 210),
        // "cloud" extracted to Modules/CloudControl (RoadToSAP Phase 1).
        new NavItem("networks",    "Nav_Networks",          Icons.Material.Filled.Hub,          "Infrastruktur",    AppRole.Viewer, 230),
        ...
        new NavItem("agent-history", "Nav_AgentHistory",    Icons.Material.Filled.Policy,       "Automatisierung",  AppRole.Viewer, 330),

        // Top-level (no group)
        new NavItem("settings",    "Nav_Settings",      Icons.Material.Filled.Settings,     "",                 AppRole.Viewer, 900),
        new NavItem("help",        "Nav_Help",              Icons.Material.Filled.MenuBook,     "",                 AppRole.Viewer, 910),
    };
}
